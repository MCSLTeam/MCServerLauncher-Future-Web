import {getUser, getUsernameByToken, hasPermission} from "~/server/utils/user";
import type {DaemonInfo, DaemonStorage, DaemonUpdate} from "@repo/commons/src/utils/daemon/daemons";
...
export async function getDaemonList(): Promise<{ [key: string]: DaemonStorage }> {
    await load;
    const daemonList = <{ [key: string]: DaemonStorage }>await storage.getItem("daemons.json");
    if (daemonList == null) {
...
export async function getDaemonListWithToken(token: string): Promise<{ [key: string]: DaemonInfo }> {
...
export async function removeDaemon(id: string) {
    await saveDaemonList(
        Object.fromEntries(
            Object.entries(await getDaemonList())
                .filter(([key, _]) => key !== id)
        )
    );
}
