...
import { setUsername } from "@repo/commons/src/utils/injections.ts";
...
export const useAccount = defineStore("account", () => {
  ...
  async function refreshTokenInfo() {
    ...
    if (res.status == "ok") {
      ...
      setUsername(tokenInfo.value.username);
    } else {
      throw res.message;
    }
  }
  ...
  async function scheduleRefresh() {
    ...
    if (loggedIn.value) {
      try {
        ...
        console.error(formatError("auth.login.failed.verify", e), e);
        await logout();
      }
    }
  }
