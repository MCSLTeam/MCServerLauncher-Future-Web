...
let lastChangedName = '';
let showMCReleases = ref(true);
let showMCSnapshots = ref(false);
let showMCOldVersions = ref(false);
...
<template>
  <ElDialog
    ...
    <ElForm
      ...
      <ElTabs @tabChange="(name) => (form.installType = <ServerInstallType>name)">
        <ElTabPane label="下载核心" :key="ServerInstallType.Download">
          <ElFormItem label="服务器类型" prop="type" required>
            <ElSelect
              ...
              <ElOptionGroup label="模组">
                ...
                <ElOption label="SpongeVanilla" value="spongevanilla"/>
                <ElOption label="SpongeForge" value="spongeforge"/>
              </ElOptionGroup>
              <ElOptionGroup label="插件">
                <ElOption label="CraftBukkit" value="craftbukkit"/>
                ...
                <ElOption label="Pufferfish" value="pufferfish"/>
                <ElOption label="Pufferfish+" value="pufferfish+"/>
                <ElOption label="Pufferfish+Purpur" value="pufferfish+purpur"/>
                <ElOption label="Purpur" value="purpur"/>
              </ElOptionGroup>
              <ElOptionGroup label="群组">
                <ElOption label="BungeeCord" value="bungeecord"/>
                <ElOption label="LightFall" value="lightfall"/>
                <ElOption label="Travertine" value="travertine"/>
            ...
            <ElSelect
              ...
              <div v-if="mcVersions">
                <ElOption
                    ...
                    :label="'最新正式版（' + mcVersions.latest.release + '）'"
                    :value="mcVersions.latest.release"/>
                <ElOption
                    :key="mcVersions.latest.snapshot"
                    :label="'最新快照版（' + mcVersions.latest.snapshot + '）'"
                ...
                <ElOption
                    ...
                    v-for="version in mcVersions.versions.filter(
										(v: any) =>
											(v.type === 'release' && showMCReleases) ||
											(v.type === 'snapshot' && showMCSnapshots) ||
											(v.type === 'old_beta' && showMCOldVersions) ||
											(v.type === 'old_alpha' && showMCOldVersions),
									)"/>
              </div>
            </ElSelect>
          </ElFormItem>
        ...
        <ElTabPane label="导入核心" :key="ServerInstallType.ImportCore">
          <ElFormItem prop="core">
            <ElUpload drag :ref="form.core" style="width: 100%">
        ...
        <ElTabPane label="导入服务端整合包" :key="ServerInstallType.ImportPack">
          <ElFormItem prop="pack">
            <ElUpload drag :ref="form.pack" style="width: 100%">
              <i class="fa fa-upload"/>
              <div class="ElUpload__text">
      ...
      <ElFormItem>
        <ElButton type="primary" @click="createInstance()">新建</ElButton>
