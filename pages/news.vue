...
const activeTab = ref(location.href.split('#').length === 2 ? location.href.split('#')[1] : 'mc-update');
const page = ref(1);

const minecraftUpdates: Ref<any> = ref(null);
const minecraftNews: Ref<any> = ref(null);

axios.get('https://launchercontent.mojang.com/javaPatchNotes.json')
    .then((res) => {
...
axios.get('https://launchercontent.mojang.com/news.json')
    .then((res) => {
          ...
          <ElTabs v-model="activeTab" @tab-change="page = 1">
            <ElTabPane :label="$t('news.tabs.mc-update')" name="mc-update">
              <div v-if="!minecraftUpdates" class="news__loading">
                <div v-loading="true" class="news__loading-icon" element-loading-background="rgba(255, 255, 255, 0.5)"/>
                <ElSkeleton animated>
                  <template #template>
                    <div class="news__cards">
                      <ElCard
                          ...
                          class="news__card"
                      >
                        <ElSkeletonItem variant="image" style="width: 100%; height: calc(100% - 50px)"/>
                        <br>
                        <ElSkeletonItem variant="h3" style="width: 50%"/>
                        <br>
                        <ElSkeletonItem class="news__date" variant="p" style="width: 25%"/>
                      </ElCard>
                    </div>
                  </template>
                </ElSkeleton>
              ...
              <div v-else class="news__tab">
                <ElDialog
                    v-for="item in minecraftUpdates.entries.slice((page - 1) * 20, page * 20)"
                    :key="item.version"
                ...
                >
                  <ElScrollbar height="500px">
                    <ElText>
                      <span class="news__dialog" v-html="item.body"/>
                    </ElText>
                  </ElScrollbar>
                </ElDialog>
                ...
                <div class="news__cards">
                  ...
                  >
                    ...
                    <h3>{{ item.title }}</h3>
                    <p class="news__date">{{
                        item.type.slice(0, 1).toUpperCase() + item.type.slice(1) + ' ' + item.version
                      }}</p>
                  </ElCard>
                </div>
                <ElPagination
            ...
            <ElTabPane :label="$t('news.tabs.mc-news')" name="mc-news">
              ...
              <div v-else class="news__tab">
                ...
                <div class="news__cards news__cards-masonry">
                  <ElCard
                      v-for="item in minecraftNews.entries.slice((page - 1) * 20, page * 20)"
                      :key="item.id"
                      class="news__card"
                  ...
                  >
                    ...
                    <h3>{{ item.title }}</h3>
                    <p class="news__desc">{{ item.text }}</p>
