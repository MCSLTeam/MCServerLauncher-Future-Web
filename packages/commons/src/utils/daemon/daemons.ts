...
export const canHideDaemonList = computed(() => Object.keys(daemonList.value).length != 0);
...
export async function connectDaemons(reconnect = false) {
    ...
    for (const key in daemonConnections.value) {
        if (!daemonList.value[key] || !daemonConnections.value[key]!.connected()) disconnectDaemon(key)
    }
...
export async function connectDaemon(id: string) {
    if (!daemonConnections.value[id]) {
        ...
        try {
            ...
            ElMessage.error(formatError('daemon.connect.failed.reconnect', e, {name: daemonList.value[id]!.name}))
            console.error(formatError('daemon.connect.failed', e, {name: daemonList.value[id]!.name}), e);
        }
    }
}
...
export async function removeDaemon(id: string, refresh = true) {
    try {
        ...
        ElMessage.error(formatError('daemon.remove.failed', e, {name: daemonList.value[id]!.name}))
        console.error(formatError('daemon.remove.failed', e, {name: daemonList.value[id]!.name}), e)
    }
}

export async function updateDaemon(id: string, info: DaemonUpdate, refresh = true) {
    try {
        if (!daemonList.value[id]) throw 'daemon-not-found';
        await daemonManager.updateDaemon(id, info);
        if (refresh) await refreshDaemon();
    } catch (e) {
        ElMessage.error(formatError('daemon.update.failed', e, {name: daemonList.value[id]!.name}))
        console.error(formatError('daemon.update.failed', e, {name: daemonList.value[id]!.name}), e)
    }
}
